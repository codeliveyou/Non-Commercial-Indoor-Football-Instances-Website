...
at_most_2_games(R_max, T1, S) :-
    S_start #= S - R_max + 1,
    findall(Slot, (scheduled(T1, _, Slot), S_start #=< Slot, Slot #=< S), TeamSlots1),
    findall(Slot, (scheduled(_, T1, Slot), S_start #=< Slot, Slot #=< S), TeamSlots2),
...
schedule_games(R_max, M) :-
    findall((T1, T2, S), (team(T1), team(T2), slot(S), T1 \= T2,
                          ...
                          at_least_m_slots_apart(M, T1, T2, S)), Games),
...
run(InputFile, R_max, M, Filename) :-
    consult(InputFile),
